...
- `lib/useRoomChat.ts` — table chat: broadcast for now, a table for later
...
- **The channel is subscribed as soon as the room opens; the microphone is
  not.** That way you can see there's a conversation to join without a card
  game opening a mic uninvited. Joining is always a tap, and the mic is
  released the moment you leave the call, the room, or the tab.
- Mute your own mic, mute anyone else just for yourself, switch on
  push-to-talk, or turn the whole feature off in Settings.
- **Connecting is not assumed to work.** Roughly a fifth of real pairs
  can't reach each other directly — mobile carriers and office networks put
  both ends behind NAT that STUN can't punch through. A failed connection
  restarts ICE, then rebuilds, then falls back to forcing everything
  through a TURN relay, and the UI says which of those is happening instead
  of showing a silent grey dot.

`NEXT_PUBLIC_TURN_URL` (with username and credential) points at your own
TURN server. Without it the Open Relay project's free public service is
used, which is shared and offers no guarantees — fine for a game night,
worth replacing if people actually use this.

## Table chat

Alongside voice, because not everyone wants to talk. Delivery and history
are two different mechanisms on purpose:

- **Realtime broadcast** carries the message, so it lands instantly with no
  replication lag and nothing to configure.
- **`room_messages`** stores it, so a refresh or a late join doesn't start
  you at a blank screen.

Splitting them means chat still works if the storage half is missing — an
un-migrated project gets a conversation that lives as long as the tab does,
rather than an error. The message id is generated by the sender, so the
broadcast copy and the stored copy are the same message and a retry can't
post twice.

Reading and posting are both gated on actually holding a seat, checked in
Postgres against the room's own `seats` JSON — the same rule voice uses, so
a room code is not a ticket into someone else's conversation. There's a row
of one-tap phrases too, because typing while it's your turn is no fun.
