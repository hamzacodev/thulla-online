    ...
    <main className="flex min-h-dvh flex-col">
      ...
      <div className="mx-auto w-full max-w-md flex-1 px-4 pb-10 pt-4">
        <div className="grid grid-cols-2 gap-2 rounded-2xl bg-white/[0.04] p-1.5">
          <Link href="/play?mode=cpu" className={`btn !min-h-11 text-sm ${mode === "cpu" ? "btn-primary" : "btn-ghost"}`}>
            🤖 vs Computer
          </Link>
          <Link href="/play?mode=friends" className={`btn !min-h-11 text-sm ${mode === "friends" ? "btn-primary" : "btn-ghost"}`}>
            👥 With Friends
          </Link>
        </div>
          ...
          <section className="mt-6">
            {needsAuth ? (
              <div className="panel p-5 text-center">
                ...
                <Link href="/play?mode=cpu" className="btn btn-ghost mt-2 w-full !text-xs">
