...
/**
 * Thulla's rules, as a component rather than a page, so the original
 * `/how-to-play` route and the game's own `/games/thulla/rules` can both
 * show them without a second copy going stale.
 */
...
export function ThullaRules() {
  return (
    <div className="mx-auto w-full max-w-md flex-1 space-y-3 px-4 pb-12 pt-4">
        ...
        <Link href="/games/thulla/play?mode=cpu" className="btn btn-primary !min-h-14 w-full text-base">
          🎮 Chalo, try it out
        </Link>
    </div>
  );
