...
-- Which game a result belongs to, and whatever only that game tracks.
-- Defaulted to 'thulla', so rows written before Bluff existed are already
-- labelled correctly. `details` holds Bluff's deck count and challenge
-- counters rather than adding columns Thulla has no use for.
...
-- ------------------------------------------------------------
-- 4b. Statistics for one game.
--     The same arithmetic as get_player_stats() with a game filter, so
--     Thulla and Bluff never see each other's numbers. Kept alongside the
